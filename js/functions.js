...
function formatPurport(text) {
    ...
    function formatPara(para) {
        ...
        para = para.replace(/\x00(\d+)\x00/g, (_, i) => {
            const phrase = placeholders[i];
            const key = phrase.toLowerCase();
            if (GLOSSARY[key]) {
                return `<span data-term="${key}">${phrase}</span>`;
            }
            return phrase;
        });
